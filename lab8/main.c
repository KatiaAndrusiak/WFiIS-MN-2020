                ...
                int n, double alfa, double beta)
{
    ...
    gsl_vector_set(d, 0, alfa);  gsl_vector_set(d, n-1, beta);

    gsl_permutation *p = gsl_permutation_alloc(n); 
    int signum; 
    gsl_vector * xd = gsl_vector_calloc(n); 
    ...
    gsl_linalg_LU_solve(A,p,d,xd);
     for(int i = 0; i < n; i++){
        m[i] = gsl_vector_get(xd, i);
    }
...
double wyznacz_Sx(double *x, double *y, double *m, 
                int n, double _x)
{
    double Sx=0;
    int i = 0;
...
int main(){
    ...
    for (int i = 0; i < n; i++){
        ...
        x[i] = xmin + delta(n)*i;
        y[i] = funkcja1(x[i]);
        poch[i] = (funkcja1(x[i] - sigma) - 2*funkcja1(x[i]) + funkcja1(x[i] + sigma)) / (sigma * sigma);
    }
    wyznacz_M(x, y, m, n, alfa, beta);
